...
-- Retrieved all employees whose salary is greater than the highest salary of the HR department

SELECT * FROM employees 
    WHERE salary > (SELECT max(salary) from employees WHERE  department_name = 'HR'); -- here sub query must return single value

-- sub query with  select must return single value
...
SELECT employee_name , salary , department_name
    FROM employees
        WHERE department_name in
         (SELECT department_name FROM employees WHERE department_name LIKE '%R%'); -- here sub query return multiple rows and it will works because in clause work with multiple rows.